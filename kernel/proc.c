...
// initthread: Initializes the main thread for a process
struct thread *
...
// thread_schd: The per-process thread scheduler
int
thread_schd(struct proc *p) {
  ...
  if (next == 0) {
    ...
    struct thread *t_old = p->current_thread;
    p->current_thread = next;
    if (t_old->trapframe) {
      *t_old->trapframe = *p->trapframe; // Save old thread's trapframe
    }
